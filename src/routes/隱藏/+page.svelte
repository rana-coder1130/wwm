    ...
    const defs = hidden
      .map((id) => {
        ...
        for (const g of checklistData.weekly.filter((i) => i.type === 'group')) {
          ...
          if (sub) return { id: sub.id, text: sub.text, sub: sub.sub, kind: 'weekly', parent: g.text } as HiddenDef;
        }
...
<div class="app-card max-w-3xl mx-auto p-6">
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center gap-2">
      ...
      <button class="btn btn-reset" on:click={confirmRestoreAll} aria-label="還原全部隱藏項目">還原全部</button>
    </div>
  </div>
...
<ConfirmModal open={confirmAll} title="還原所有隱藏項目" message="確定要將所有已隱藏項目還原嗎？" confirmLabel="還原" cancelLabel="取消" on:confirm={onConfirmAll} on:cancel={onCancelAll} />
